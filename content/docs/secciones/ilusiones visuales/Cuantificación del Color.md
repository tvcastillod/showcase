...
El dithering aleatorio fue el primer intento de remediar los inconvenientes del thresholding. Cada valor de píxel se compara con un umbral aleatorio, lo que da como resultado una imagen estática. Aunque este método no genera artefactos de patrón, el ruido tiende a inundar el detalle de la imagen. Es análogo a la práctica del mezzotinting. Este es el tipo de algoritmos de dithering.  No es realmente aceptable como método de producción, pero es muy sencillo de describir y aplicar.  Para cada valor de la imagen, simplemente se genera un número aleatorio 1..256; si es mayor que el valor de la imagen en ese punto, se traza el punto en blanco, si no, se traza en negro.  Y ya está.  Esto genera una imagen con mucho "ruido blanco", que se parece a la "nieve" de las imágenes de TV.  Aunque la imagen producida es muy imprecisa y ruidosa, está libre de "artefactos", que son fenómenos producidos por el procesamiento digital de la señal.
</p>
